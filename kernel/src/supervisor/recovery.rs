...
use super::store_manager::StoreMicroReboot;
...
impl<'engine> PrototypeRuntime<'engine> {
    ...
    pub(crate) fn recover_procfs_store_after_trap(
        ...
        self.drop_store_instance(reboot.store)
            .map_err(ServiceCallError::Invalid)?;
        ...
        self.rebind_store_instance(reboot.store)
            .map_err(ServiceCallError::Invalid)?;
        self.finish_store_micro_reboot(reboot)
            .map_err(ServiceCallError::Invalid)?;
        Ok(())
    }
    ...
    fn finish_store_micro_reboot(&mut self, reboot: StoreMicroReboot) -> Result<(), &'static str> {
        self.store_manager
            .finish_micro_reboot(&mut self.semantic, reboot.store)
    }
}
