...
That accepted v1 mapping explicitly declares `adapter_qualification=false`; Nexus
...
Separately, the bounded 0.1 implementation target now separates two explicit
profiles on
...
These protocol-2.1 controls are current engineering scope; they do not expand
the marked v2 semantic contract above without a separately bound
implementation and evidence revision.
