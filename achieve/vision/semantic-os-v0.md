# Legacy Semantic OS Architecture Note

Status: superseded by `../specs/semantic-virtual-isa-v0/00-overview.md`.

This file keeps the old "Semantic OS" entry point so older references do not
...
```text
VMOS is a capability-oriented Semantic Virtual ISA for portable system
semantics.
```
...
TargetArtifactImage is the VMOS target-side artifact envelope.
.cwasm or Wasmtime serialized payloads are replaceable code payload variants,
not the VMOS semantic ABI.
```
