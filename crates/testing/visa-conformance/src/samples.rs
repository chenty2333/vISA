...
use crate::{
    ...
    hash::sha256_hex,
    ltp::{LTP_VISA_TRACE_SCHEMA_VERSION, ltp_subset_result},
...
pub fn sample_report(catalog: &[TestSpec]) -> ConformanceReport {
    ConformanceReport {
        ...
        generated_by: "visa-conformance sample-report".to_string(),
        results: catalog
            .iter()
            .map(|spec| TestResult {
...
pub fn sample_ltp_report() -> ConformanceReport {
    ...
    ConformanceReport {
        ...
        generated_by: "visa-conformance sample-ltp-report".to_string(),
        results: catalog
            .iter()
            .map(|spec| {
                let raw_log = format!(
                ...
                let trace = format!(
                    "{{\"schema_version\":\"{}\",\"spec_id\":\"{}\",\"case_id\":\"{}_smoke_01\",\"test_binary\":\"samples/{}\",\"runner\":\"visa-linux-personality\",\"entered_visa_execution\":true,\"linux_personality_dispatch\":true,\"syscalls_observed\":1,\"service_syscalls_observed\":1,\"exit_status\":0,\"runner_status\":0,\"raw_log_uri\":\"samples/{}.log\",\"serial_log_uri\":\"samples/{}.serial.log\"}}\n",
                ...
                result.evidence_artifacts.push(EvidenceArtifact {
                    ...
                    uri: format!("samples/{}.visa-trace.jsonl", spec.id),
                    sha256: sha256_hex(trace.as_bytes()),
                    description: "synthetic vISA Linux personality trace metadata for schema validation"
                        .to_string(),
                });
                result
            })
...
pub fn sample_performance_report() -> ConformanceReport {
    ConformanceReport {
        ...
        generated_by: "visa-conformance sample-performance-report".to_string(),
