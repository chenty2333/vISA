//! Historical core-Wasm Wasmtime adapter comparison oracle.
//!
//! Stage 1 production uses the Component Model adapter in `visa_wasmtime`.
//! This crate wraps a Wasmtime engine around the historical runtime, executing Wasm guest
//! modules while routing hostcalls through the contract-aware hostcall dispatch
//! path.  It is a `std` crate and does not modify the `no_std` visa_runtime core.
...
fn validate_module_hostcall_imports(
    ...
    for import in module.imports() {
        ...
        let Some(number) = name.strip_prefix("hostcall_") else {
            return Err(WasmVisaError::InvalidHostcallImport(format!("visa::{name}")));
        };
        let number = number
            .parse::<u32>()
            .map_err(|_| WasmVisaError::InvalidHostcallImport(format!("visa::{name}")))?;
        if number == 0 {
            return Err(WasmVisaError::InvalidHostcallNumber(number));
        }
...
mod tests {
    ...
    use visa_runtime::{VisaArtifactDescriptor, VisaHostcallValue, VisaRuntimeConfig};
    ...
    fn temp_wasmtime_test_dir(name: &str) -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("visa-wasmtime-{name}-{}-{nonce}", std::process::id()))
    }
    ...
    fn wasm_module_bytes() -> Vec<u8> {
        wat::parse_str(
            r#"(module
  (import "visa" "hostcall_1" (func $hc1 (param i64 i64 i64 i64 i64 i64) (result i64)))
  ...
  (func (export "visa_start") (result i64)
    ...
    fn visa_native_negative_timer_arg_wasm() -> Vec<u8> {
        wat::parse_str(
            r#"(module
  (import "visa" "hostcall_1" (func $timer_arm (param i64 i64 i64 i64 i64 i64) (result i64)))
  (func (export "visa_start") (result i64)
    i64.const 10
    i64.const -1
    i64.const 1
  ...
  (func (export "probe_copyin_byte") (result i64)
    ...
    fn run_executes_native_visa_memory_backed_console_hostcall() {
        ...
        let personality = visa_runtime::personality::native::VisaNativePersonality::new(
            "native-visa",
    ...
    fn run_executes_native_visa_full_substrate_hostcall_abi() {
        ...
        let conformance_report = visa_conformance::ConformanceReport {
            ...
            target: "visa-wasmtime-unit".to_string(),
            generated_by: "visa-wasmtime-test".to_string(),
    ...
    fn repeated_runs_report_only_current_hostcalls() {
        ...
        assert!(second.events.iter().any(|event| {
            matches!(event, visa_runtime::VisaRuntimeEvent::ArtifactParsed { artifact_id: 10, .. })
        }));
        assert!(!second.events.iter().any(|event| {
            matches!(event, visa_runtime::VisaRuntimeEvent::ArtifactParsed { artifact_id: 9, .. })
        }));
        assert!(
  ...
  (func (export "entry") (result i64)
    ...
    fn zero_hostcall_wasm() -> Vec<u8> {
        wat::parse_str(
            r#"(module
  (import "visa" "hostcall_0" (func $hc0 (param i64 i64 i64 i64 i64 i64) (result i64)))
  (func (export "entry") (result i64)
    i64.const 1
    i64.const 2
    i64.const 3
  ...
  (func (export "entry")
    ...
    fn wrong_hostcall_signature_wasm() -> Vec<u8> {
        wat::parse_str(
            r#"(module
  (import "visa" "hostcall_1" (func $hc1 (param i32) (result i32)))
  (func (export "entry") (result i32)
    ...
    #[test]
    fn run_rejects_zero_descriptor_hostcall_number_before_activation() {
        let rt = VisaRuntime::new(VisaRuntimeConfig::for_profile(SubstrateProfile::GuestFrontend));
        let substrate = TestSubstrate::default();
        let mut executor = WasmVisaExecutor::new(rt, Box::new(substrate));
        let store_count_before = executor.runtime().semantic().store_count();
        let artifact = fake_artifact(&REQUIRED_SECTIONS, &zero_hostcall_wasm());
        let desc = VisaArtifactDescriptor::new(
            9,
            "test",
            "test-artifact",
        ...
        let err = executor
            .run(VisaArtifactInput { bytes: &artifact, descriptor: desc }, "entry")
            .expect_err("zero descriptor hostcall number must fail validation");
        assert!(
    ...
    #[test]
    fn run_rejects_zero_import_hostcall_number_before_activation() {
        let rt = VisaRuntime::new(VisaRuntimeConfig::for_profile(SubstrateProfile::GuestFrontend));
        let substrate = TestSubstrate::default();
        let mut executor = WasmVisaExecutor::new(rt, Box::new(substrate));
        let store_count_before = executor.runtime().semantic().store_count();
        ...
        let err = executor
            .run(VisaArtifactInput { bytes: &artifact, descriptor: desc }, "entry")
            .expect_err("zero import hostcall number must fail validation");
        assert!(
            matches!(err, WasmVisaError::InvalidHostcallNumber(0)),
            "expected InvalidHostcallNumber, got: {err}"
        );
