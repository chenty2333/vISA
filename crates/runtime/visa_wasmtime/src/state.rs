...
impl PortableComponentState {
    ...
    pub(crate) fn encode(state: &WitComponentState) -> Result<Self, StateCodecError> {
    ...
    pub(crate) fn decode(&self) -> Result<WitComponentState, StateCodecError> {
        decode(&self.0)
    }

    pub(crate) fn phase(&self) -> Result<WorkloadPhase, StateCodecError> {
        self.decode().map(|state| public_phase(state.phase))
    }
}
...
impl From<WitComponentState> for ComponentStatus {
    fn from(state: WitComponentState) -> Self {
        Self {
...
fn decode(bytes: &[u8]) -> Result<WitComponentState, StateCodecError> {
...
const fn phase_tag(phase: WitPhase) -> u8 {
    match phase {
        WitPhase::Armed => 0,
        WitPhase::Frozen => 1,
        WitPhase::Completed => 2,
        WitPhase::Cancelled => 3,
    }
}

const fn public_phase(phase: WitPhase) -> WorkloadPhase {
    match phase {
...
mod tests {
    ...
    fn component_state_encoding_is_deterministic_and_round_trips() {
        ...
        assert_eq!(decoded.phase, WitPhase::Frozen);
