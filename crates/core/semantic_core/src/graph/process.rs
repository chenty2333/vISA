...
impl SemanticGraph {
    ...
    pub fn create_process(
        &mut self, pid: u32, parent_pid: Option<u32>, pgid: u32, sid: u32,
        thread_group: ContractObjectRef, exit_signal: Option<u8>,
    ) -> ProcessId {
        let id = self.domains.process.next_process_id;
        self.domains.process.next_process_id = id.max(id + 1);
        self.create_process_with_id(id, pid, parent_pid, pgid, sid, thread_group, exit_signal);
        id
    }

    pub fn create_process_with_id(
        &mut self, id: ProcessId, pid: u32, parent_pid: Option<u32>,
        pgid: u32, sid: u32, thread_group: ContractObjectRef, exit_signal: Option<u8>,
    ) -> bool {
    ...
    pub fn transition_process_state(
        &mut self, id: ProcessId, generation: Generation, new_state: ProcessState,
    ) -> bool {
        let Some(record) = self.domains.process.processes.iter_mut().find(|r| {
        ...
        record.generation += 1;
        self.event_log.push("process", EventKind::ProcessStateChanged {
            pid: record.pid,
            old_state: old_str,
            new_state: new_str,
        });
    ...
    pub fn create_thread(
        &mut self, tid: u32, task_id: u64, process: ContractObjectRef,
        aspace: ContractObjectRef, fd_table: ContractObjectRef,
        credential: ContractObjectRef, thread_group: ContractObjectRef,
    ) -> ThreadId {
        let id = self.domains.process.next_thread_id;
        self.domains.process.next_thread_id = id.max(id + 1);
        self.create_thread_with_id(id, tid, task_id, process, aspace, fd_table, credential, thread_group);
        id
    }

    pub fn create_thread_with_id(
        &mut self, id: ThreadId, tid: u32, task_id: u64, process: ContractObjectRef,
        aspace: ContractObjectRef, fd_table: ContractObjectRef,
        credential: ContractObjectRef, thread_group: ContractObjectRef,
    ) -> bool {
        ...
        self.domains.process.threads.push(ThreadRecord {
            id, tid, task_id, process, aspace, fd_table, credential, thread_group,
    ...
    pub fn create_thread_group(
        &mut self, tgid: u32, leader: ContractObjectRef,
    ) -> ThreadGroupId {
        let id = self.domains.process.next_thread_group_id;
        self.domains.process.next_thread_group_id = id.max(id + 1);
        self.create_thread_group_with_id(id, tgid, leader);
        id
    }

    pub fn create_thread_group_with_id(
        &mut self, id: ThreadGroupId, tgid: u32, leader: ContractObjectRef,
    ) -> bool {
    ...
    pub fn create_fd_table(
        &mut self, owner_thread_group: ContractObjectRef, shared: bool,
    ) -> FdTableId {
        let id = self.domains.process.next_fd_table_id;
        self.domains.process.next_fd_table_id = id.max(id + 1);
        self.create_fd_table_with_id(id, owner_thread_group, shared);
        id
    }

    pub fn create_fd_table_with_id(
        &mut self, id: FdTableId, owner_thread_group: ContractObjectRef, shared: bool,
    ) -> bool {
    ...
    pub fn create_credential(
        &mut self, owner_process: ContractObjectRef,
        ...
        self.create_credential_with_id(id, owner_process,
            uid, euid, suid, fsuid, gid, egid, sgid, fsgid);
        id
    }

    pub fn create_credential_with_id(
        &mut self, id: CredentialId, owner_process: ContractObjectRef,
        uid: u32, euid: u32, suid: u32, fsuid: u32,
        gid: u32, egid: u32, sgid: u32, fsgid: u32,
    ) -> bool {
    ...
    pub fn record_credential_transition(
        &mut self, from: ContractObjectRef, to: ContractObjectRef,
        kind: CredentialTransitionKind, broadcast: bool,
        ...
        self.domains.process.credential_transitions.push(CredentialTransitionRecord {
            id, from_credential: from, to_credential: to,
    ...
    pub fn check_process_invariants(&self) -> Vec<String> {
        ...
        for process in &self.domains.process.processes {
            if process.id == 0 { violations.push(format!("process {}: id=0", process.pid)); }
            if process.generation == 0 { violations.push(format!("process {}: gen=0", process.pid)); }
            if process.thread_group.id == 0 {
                violations.push(format!("process {}: missing thread_group", process.pid));
            }
            if !self.domains.process.thread_groups.iter().any(|tg| tg.object_ref() == process.thread_group) {
                violations.push(format!("process {}: dangling thread_group", process.pid));
            }
        }
        for thread in &self.domains.process.threads {
            if thread.id == 0 { violations.push(format!("thread {}: id=0", thread.tid)); }
            if thread.generation == 0 { violations.push(format!("thread {}: gen=0", thread.tid)); }
        ...
        for cred in &self.domains.process.credentials {
            if cred.id == 0 { violations.push(format!("cred {}: id=0", cred.id)); }
            if cred.generation == 0 { violations.push(format!("cred {}: gen=0", cred.id)); }
        }
        for fd_table in &self.domains.process.fd_tables {
            if fd_table.id == 0 { violations.push(format!("fd_table {}: id=0", fd_table.id)); }
            if fd_table.generation == 0 { violations.push(format!("fd_table {}: gen=0", fd_table.id)); }
        }
        violations
    }
}
