...
use super::{
    ...
    types::{
        ...
        ProcessRuntimeStateKind, PtraceAttachment, RLIMIT_CPU, RLIMIT_NOFILE, RLIMIT_STACK, Rlimit,
        RuntimeClockAdjustmentState, SeccompAuditAction, SeccompAuditRecord, SeccompMode,
        SeccompNotification, SeccompTraceEvent, ServiceCallError, SigAction, SocketPairState,
...
pub(crate) struct PrototypeRuntime<'engine> {
    ...
    pub(super) timerfds: Vec<TimerFdState>,
    pub(super) next_timerfd_id: u64,
    ...
    pub(super) seccomp_trace_listener_enabled: bool,
    pub(super) next_seccomp_trace_id: u64,
    pub(super) seccomp_trace_events: Vec<SeccompTraceEvent>,
    pub(super) ptrace_attachments: Vec<PtraceAttachment>,
    ...
    pub(super) generic_mmap_cursor: u64,
...
impl<'engine> PrototypeRuntime<'engine> {
    pub(super) fn new(engine: &'engine RuntimeOnlyExecutor) -> Result<Self, &'static str> {
        ...
        Ok(Self {
            ...
            seccomp_trace_listener_enabled: false,
            next_seccomp_trace_id: 1,
            seccomp_trace_events: Vec::new(),
            ptrace_attachments: Vec::new(),
            seccomp_audit: Vec::new(),
            generic_mmap_regions: Vec::new(),
            generic_locked_mmap_ranges: Vec::new(),
            generic_mlock_future_pids: Vec::new(),
            generic_mmap_cursor: 0,
            fault: None,
...
fn seccomp_audit_action(decision: SeccompDecision) -> Option<(SeccompAuditAction, u16)> {
    match decision {
        ...
        SeccompDecision::Trace { data } => Some((SeccompAuditAction::Trace, data)),
