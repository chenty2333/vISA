...
impl LinuxFrontend {
    ...
    pub(crate) fn dispatch(&mut self, ctx: SyscallContext) -> Result<u64, &'static str> {
        ...
        let step = match ctx.nr {
            ...
            SYS_DUP2 => self.plan_dup(a0, a1, 0, 1),
            SYS_DUP3 => self.plan_dup(a0, a1, a2, 2),
            ...
            SYS_GETSOCKNAME => self.plan_getsockname(a0, a1, a2),
            SYS_GETPEERNAME => self.plan_getpeername(a0, a1, a2),
            ...
            SYS_RT_SIGPENDING => self.plan_rt_sigpending(a0, a1),
            ...
            SYS_GETUID => self.plan_simple(PlanKind::GetUid),
            SYS_GETGID => self.plan_simple(PlanKind::GetGid),
            SYS_GETEUID => self.plan_simple(PlanKind::GetEuid),
            SYS_GETEGID => self.plan_simple(PlanKind::GetEgid),
            SYS_SETUID => self.plan_setuid(a0),
    ...
    fn plan_getsockname(&mut self, fd: u64, addr: u64, addr_len: u64) -> PackedStep {
        self.reset_plan(PlanKind::GetSockName, [fd, addr, addr_len, 0, 0, 0]);
        PackedStep::plan(PlanKind::GetSockName)
    }

    fn plan_getpeername(&mut self, fd: u64, addr: u64, addr_len: u64) -> PackedStep {
        self.reset_plan(PlanKind::GetPeerName, [fd, addr, addr_len, 0, 0, 0]);
    ...
    fn plan_clock_gettime(&mut self, clock_id: u64, timespec_ptr: u64) -> PackedStep {
        self.reset_plan(PlanKind::ClockGettime, [clock_id, timespec_ptr, 0, 0, 0, 0]);
    ...
    fn plan_get_robust_list(&mut self, pid: u64, head_ptr: u64, len_ptr: u64) -> PackedStep {
        self.reset_plan(PlanKind::GetRobustList, [pid, head_ptr, len_ptr, 0, 0, 0]);
    ...
    fn plan_close_range(&mut self, first: u64, last: u64, flags: u64) -> PackedStep {
        self.reset_plan(PlanKind::CloseRange, [first, last, flags, 0, 0, 0]);
        PackedStep::plan(PlanKind::CloseRange)
    }

    fn plan_dup(&mut self, old_fd: u64, new_fd: u64, flags: u64, mode: u64) -> PackedStep {
        self.reset_plan(PlanKind::Dup, [old_fd, new_fd, flags, mode, 0, 0]);
    ...
    fn plan_rt_sigpending(&mut self, set: u64, sigsetsize: u64) -> PackedStep {
        self.reset_plan(PlanKind::RtSigpending, [set, sigsetsize, 0, 0, 0, 0]);
    ...
    fn plan_setuid(&mut self, uid: u64) -> PackedStep {
        self.reset_plan(PlanKind::SetUid, [uid, 0, 0, 0, 0, 0]);
        PackedStep::plan(PlanKind::SetUid)
    }

    fn plan_setgid(&mut self, gid: u64) -> PackedStep {
        self.reset_plan(PlanKind::SetGid, [gid, 0, 0, 0, 0, 0]);
        PackedStep::plan(PlanKind::SetGid)
    }

    fn plan_setreuid(&mut self, ruid: u64, euid: u64) -> PackedStep {
        self.reset_plan(PlanKind::SetReUid, [ruid, euid, 0, 0, 0, 0]);
    ...
    fn setsockopt_u32_value(
        ...
        if level != SOL_SOCKET
            || !matches!(
                optname,
                SO_REUSEADDR | SO_REUSEPORT | SO_KEEPALIVE | SO_SNDBUF | SO_RCVBUF
            )
        {
            return Ok(0);
        }
