...
impl DevfsService {
    pub(crate) fn new(engine: &SupervisorEngine) -> Result<Self, &'static str> {
        ...
        Ok(Self { io, lookup, node_kind, list_dir, read_device, write_device })
    }
    ...
    pub(crate) fn lookup(
        ...
        expect_ok(
            self.io
                .call(&self.lookup, (path_len, inject_fault as u32), "devfs_service trapped")
        ...
        .ok_or(ServiceCallError::Invalid("devfs_service returned an invalid node kind"))
    }
    ...
    pub(crate) fn list_dir(
        ...
        let len = expect_len(
            self.io
                .call(&self.list_dir, (path_len, inject_fault as u32), "devfs_service trapped")
                .map_err(ServiceCallError::Trap)?,
        )?;
