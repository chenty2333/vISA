...
fn dispatch_futex(
    ...
    if command == FUTEX_CMP_REQUEUE {
        return plan_futex_cmp_requeue(key, val, timeout_ptr, timeout_len, current_word);
    }
...
mod tests {
    ...
    #[test]
    fn futex_cmp_requeue_checks_source_word_before_planning() {
        let _guard = test_guard();
        let src_word = 0x44u32;
        let (src, dst) = unsafe {
            let base = core::ptr::addr_of_mut!(ARG_BUFFER) as *mut u8;
