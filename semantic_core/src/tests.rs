...
fn v12_resumed_vector_context() -> SemanticGraph {
    let mut graph = v8_saved_vector_context_with_decision();
    let resumed = graph.apply_envelope(CommandEnvelope::new(
        3,
        "v12-test",
...
#[test]
fn simd_runtime_v12_context_switch_benchmark_records_vector_overhead() {
...
#[test]
fn simd_runtime_v12_rejects_overhead_budget_violation() {
    let mut graph = v12_resumed_vector_context();
...
#[test]
fn simd_runtime_v12_invariants_reject_overhead_drift() {
    let mut graph = v12_resumed_vector_context();
...
fn x1_integrated_smp_network_fault_snapshot() -> ContractGraphSnapshot {
    let backend = ContractObjectRef::new(ContractObjectKind::VirtioNetBackendObject, 1553, 1);
...
#[test]
fn integrated_runtime_x1_contract_graph_accepts_network_fault_under_smp() {
    let violations = validate_contract_graph(&x1_integrated_smp_network_fault_snapshot());
    assert_eq!(violations, Vec::new());
}

#[test]
fn integrated_runtime_x1_rejects_stale_or_incomplete_evidence() {
...
#[test]
fn integrated_runtime_x1_contract_graph_rejects_generation_drift() {
