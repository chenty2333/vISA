...
impl SemanticGraph {
    ...
    pub(crate) fn validate_vector_state(
        ...
        if self
            .vector_states
            .iter()
            .any(|record| record.id == vector_state)
        {
    ...
    pub fn record_vector_state_with_id(
        ...
        let generation = 1;
        self.next_vector_state_id = self
            .next_vector_state_id
            .max(vector_state.saturating_add(1));
    ...
    pub(crate) fn corrupt_vector_state_owner_activation_generation_for_test(
        ...
        if let Some(record) = self
            .vector_states
            .iter_mut()
            .find(|record| record.id == vector_state)
        {
