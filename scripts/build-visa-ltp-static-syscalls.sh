...
usage() {
    cat >&2 <<'EOF'
usage: scripts/build-visa-ltp-static-syscalls.sh <ltp-source-dir> <binary-root> <manifest> [limit]

Builds LTP syscall test binaries as static ELF files in a Docker toolchain,
copies a manifest-sized candidate set into <binary-root>, and writes a vISA LTP
manifest consumable by scripts/run-visa-ltp-manifest.sh.

The build happens under:

  ${VISA_LTP_BUILD_DIR:-${XDG_CACHE_HOME:-$HOME/.cache}/visa-ltp/build-src}
...
manifest="$3"
limit="${4:-500}"
cache_root="${XDG_CACHE_HOME:-$HOME/.cache}/visa-ltp"
build_dir="${VISA_LTP_BUILD_DIR:-$cache_root/build-src}"
build_backend="${VISA_LTP_BUILD_BACKEND:-docker}"
...
case "$build_backend" in
    docker|host) ;;
    *)
        echo "VISA_LTP_BUILD_BACKEND must be docker or host" >&2
        exit 64
        ;;
esac
...
if [[ "$build_backend" == "docker" ]]; then
    docker run --rm -v "$build_dir:/src" -w /src debian:stable-slim bash -lc '
        ...
        if [ ! -x ./configure ]; then
            make autotools >/src/visa-ltp-autotools.log
        fi
        ./configure CFLAGS="-O2 -D_GNU_SOURCE" LDFLAGS="-static" >/src/visa-ltp-configure.log
    ...
    (
        cd "$build_dir"
        if [[ ! -x ./configure ]]; then
            make autotools >"$build_dir/visa-ltp-autotools.log"
        fi
        ./configure CFLAGS="-O2 -D_GNU_SOURCE -std=gnu17 -include sys/sysinfo.h" LDFLAGS="-static" \
            >"$build_dir/visa-ltp-configure.log"
        python3 - <<'PY'
