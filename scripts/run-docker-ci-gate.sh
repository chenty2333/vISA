...
system-stock-zstd is host-special-cased for the same reason and retains only
its compact matrix and input build receipts in the CI artifact root.
...
while [[ "$#" -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            exit 0
            ;;
        fast|full|system|system-jco-node|system-stage2|system-stage2-strict|system-stage3a|system-stage3a-cross-runtime|system-stage3b|system-stage3|system-wanco-carrier|system-stock-zstd|system-stock-sqlite|system-stage4-target|system-stage4-isa|system-stage4|system-joint-handoff)
            if [[ -n "$tier" ]]; then
...
if [[ "$tier" == system-wanco-carrier || "$tier" == system-stock-zstd \
