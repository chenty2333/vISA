...
usage() {
    cat >&2 <<'EOF'
usage: scripts/run-vmos-ltp-single.sh <spec-id> <case-id> <test-elf> <raw-log> <trace-log> <serial-log>

Builds/runs the VMOS QEMU runner with <test-elf> embedded as the Linux
personality user ELF, captures serial output, extracts or synthesizes a raw LTP
case log, and emits a VMOS Linux personality execution trace.

The test ELF must be loadable by the current VMOS Linux ELF frontend. Ordinary
...
if [[ ! -f "$test_elf" ]]; then
    echo "VMOS LTP test ELF not found: $test_elf" >&2
    exit 66
fi
...
cargo run --quiet -p vmos-conformance -- \
    ltp-vmos-trace-from-serial "$spec_id" "$case_id" "$test_elf" "$raw_uri" "$serial_uri" \
