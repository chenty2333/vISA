...
func main() {
	if len(os.Args) != 3 {
		fatalf("usage: wacogo-upstream-probe COMPONENT WORLD_WIT")
	}
	...
	const expectedFailure = `wacogo: instantiate component: arg "import-type-kv-error" references unresolved type 24`
	...
	for attempt := range failures {
		...
		if err == nil {
			...
			fatalf(
				"unchanged component instantiation unexpectedly passed on attempt %d; qualification must stop for review",
				attempt+1,
			)
		}
		if instance != nil {
			_ = instance.Close(ctx)
			fatalf(
				"unchanged component instantiation returned both an instance and an error on attempt %d",
				attempt+1,
			)
		}
		failures[attempt] = err.Error()
		if failures[attempt] != expectedFailure {
			fatalf(
				"unexpected unchanged-component instantiation failure on attempt %d: got %q, want %q",
				attempt+1,
				failures[attempt],
				expectedFailure,
			)
	...
	if failures[0] != failures[1] {
		fatalf(
			"unchanged-component diagnostics were not deterministic: first=%q second=%q",
			failures[0],
			failures[1],
		)
	}
	...
	fmt.Printf(
		"unchanged-component-instantiation=unsupported attempts=2 exact=true deterministic=true error=%q\n",
		failures[0],
	)
